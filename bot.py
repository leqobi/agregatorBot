...
API_TOKEN = os.environ.get("API_TOKEN", "ТВОЙ_ТОКЕН")
...
def get_main_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="💰 Заработок", callback_data="earn"),
        ],
        [
            InlineKeyboardButton(text="🛠 Сервисы", callback_data="services"),
            InlineKeyboardButton(text="📦 Разное", callback_data="other"),
        ],
        [InlineKeyboardButton(text="📞 Написать создателю", callback_data="support")]
    ])
    return keyboard
...
async def reply_to_user(message: types.Message, command: CommandObject):
    if message.from_user.id != ADMIN_ID:
        await message.answer("⛔ Только для админа.")
        return
    if not command.args:
...
# ------------------- Обработка текстовых сообщений (должна быть ПОСЛЕ команд) -------------------
@dp.message(F.text)
async def handle_text(message: types.Message):
    user_id = message.from_user.id
    text = message.text

    # Сообщения от самого админа не трогаем
    if user_id == ADMIN_ID:
        return
